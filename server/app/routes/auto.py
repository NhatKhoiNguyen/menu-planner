...
def format_ingredients(ingredients, include_amount=True):
    if include_amount:
        return "\n".join(
            f"- {i['name']}: {i['amount']}"
            for i in ingredients
            if i.get("name") and i.get("amount")
        )

    return "\n".join(f"- {i['name']}" for i in ingredients if i.get("name"))
...
def auto_energy(current_user):
    ...
    if not ingredients:
        return jsonify({"error": "Thiếu nguyên liệu"}), 400

    prompt = f"""
Ước tính tổng năng lượng của món ăn.
...
"""
    ...
    try:
        ...
        print(e)
        return jsonify({"error": "Không thể tính calories"}), 500
...
def auto_price(current_user):
    ...
    prompt = f"""
Ước tính giá tiền món ăn theo giá thực phẩm Việt Nam hiện nay.
...
"""
    ...
    try:
        ...
        print(e)
        return jsonify({"error": "Không thể tính giá"}), 500
...
{format_ingredients(ingredients, False)}

Chọn những tag phù hợp nhất.
"""
    ...
    try:
        ...
        tags = [t for t in response.parsed if t in ALLOWED_TAGS]

        return jsonify({"tags": tags})

    except Exception as e:
        print(e)
        return jsonify({"error": "Không thể gợi ý tag"}), 500
...
Xác định các chất dễ gây dị ứng.
"""
    ...
    try:
        ...
        allergens = [a for a in response.parsed if a in ALLERGENS]

        return jsonify({"allergens": allergens})

    except Exception as e:
        print(e)
        return jsonify({"error": "Không thể xác định dị ứng"}), 500
